...
namespace VetService.Domain.Managers
{
    public partial class ManagerSymptom:IManagerSymptom
    {
        ...
        public SymptomApiModel GetSymptomByAppId(int id)
        {
            ...
            {
                var symptom = _symptomRepository.GetByAppId(id);
        ...
        public async Task<IEnumerable<SymptomApiModel>> GetSymptomByAppIdAsync(int id)
        {
            var symptoms = (await _symptomRepository.GetByAppIdAsync(id)).ConvertAll();
            foreach (var symptom in symptoms)
            {
                var cacheEntryOptions =
                    new MemoryCacheEntryOptions().SetSlidingExpiration(TimeSpan.FromSeconds(604800));
