...
namespace VetService.Domain.Managers
{
    public partial class ManagerTest:IManagerTest
    {
        ...
        public TestApiModel GetTestByAppId(int id)
        {
            ...
            {
                var test = _testRepository.GetByAppId(id);
        ...
        public async Task<IEnumerable<TestApiModel>> GetTestByAppIdAsync(int id)
        {
            var tests = (await _testRepository.GetByAppIdAsync(id)).ConvertAll();
            foreach (var test in tests)
            {
                var cacheEntryOptions =
                    new MemoryCacheEntryOptions().SetSlidingExpiration(TimeSpan.FromSeconds(604800));
